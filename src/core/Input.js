  ...
  return validationKey ? (<View style={[blockStyles]}>
    ...
    {!isValid && (
      <View style={{ flexDirection: "row", columnGap: 6, alignItems: "center" }}>
        <Icon type={"ExclamationMark"} size={20} />
...
const DefaultFlow = React.forwardRef(({
  ...
  return (
    <Animated.View
      style={[s.container, {
        borderColor: validationKey ? (defaultValue ? (isValid ? "rgba(178, 178, 178, 1)" : Colors.red) : "rgba(178, 178, 178, 1)") : "rgba(178, 178, 178, 1)",
    ...
    >
      ...
      {validationKey === "password" ? (<Button onPress={() => setVisibility(!visibility)}>
        <Icon
          ...
          size={20}
          color={defaultValue ? Colors.lightGray : Colors.lightGray}
        />
      </Button>) : null}
    </Animated.View>);
});
...
const s = StyleSheet.create({
  ...
  input: {
    fontSize: 16, fontWeight: "400", color: "white", padding: 4, flex: 1,
  },
  error: {
    color: Colors.red,
    ...margin(12, 0, 10, 0),
